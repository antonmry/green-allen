...
### Build Allen with GPU support

The container already bundles CUDA 12.6 and GCC 13 through `gcc-toolset-13`. To produce a CUDA-enabled binary:

```bash
source /opt/rh/gcc-toolset-13/enable

# Reuse the full-stack checkouts generated by the CPU build (avoids new Git fetches).
export LHCBROOT=/workspace/Allen/build/external/LHCb
export GAUDIROOT=/workspace/Allen/build/external/Gaudi
export PARAMFILESROOT=/workspace/Allen/build/external/ParamFiles

cmake -S Allen -B Allen/build_cuda -DSTANDALONE=ON -DTARGET_DEVICE=CUDA
cmake --build Allen/build_cuda --target Allen -j"$(nproc)"
```

If you prefer not to export the environment variables each time, symlink the cached externals:

```bash
ln -sfn ../build/external Allen/build_cuda/external
```
...
Ensure the container sees a GPU (`nvidia-smi` should succeed) and execute the CUDA build:
